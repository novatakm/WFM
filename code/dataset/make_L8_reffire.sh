...
do
    if [ -f $RAWFIRE_DIR/$rawfire_dir/L8REFFIRE.shp ]; then
        ...
        v.what.rast map=${reffire} type=point raster=${L8_data_id}_L6 column="l6" --o
        v.what.rast map=${reffire} type=point raster=${L8_data_id}_L7 column="l7" --o
        v.what.rast map=${reffire} type=point raster=${L8_data_id}_PR6 column="r6" --o
        v.what.rast map=${reffire} type=point raster=${L8_data_id}_PR7 column="r7" --o
